...
export const MCPAddServer: React.FC<MCPAddServerProps> = ({
  ...
  const handleAddStdioServer = async () => {
    if (!stdioName.trim()) {
      onError(t('mcpAddServer.nameRequired'));
      return;
    }
    
    if (!stdioCommand.trim()) {
      onError(t('mcpAddServer.commandRequired'));
      return;
    }
  ...
  const handleAddSseServer = async () => {
    ...
    if (!sseUrl.trim()) {
      onError(t('mcpAddServer.urlRequired'));
      return;
    }
      ...
      <Tabs value={transport} onValueChange={(v) => setTransport(v as "stdio" | "sse")}>
        <TabsList className="grid w-full grid-cols-2 max-w-sm mb-6">
          <TabsTrigger value="stdio" className="gap-2">
            ...
            {t('mcpAddServer.typeStdio')}
          </TabsTrigger>
          <TabsTrigger value="sse" className="gap-2">
            <Globe className="h-4 w-4 text-emerald-500" />
            {t('mcpAddServer.typeSse')}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="stdio" className="space-y-6">
          <Card className="p-6 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="stdio-name">{t('mcpAddServer.name')}</Label>
                <Input
                  id="stdio-name"
                ...
                <p className="text-xs text-muted-foreground">
                  {t('mcpAddServer.nameDescription') || 'A unique name to identify this server'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-command">{t('mcpAddServer.command')}</Label>
                <Input
                  id="stdio-command"
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {t('mcpAddServer.commandDescription') || 'The command to execute the server'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-args">{t('mcpAddServer.args')}</Label>
                <Input
                  id="stdio-args"
                  placeholder={t('placeholders.serverArgs')}
                  value={stdioArgs}
                  onChange={(e) => setStdioArgs(e.target.value)}
                  className="font-mono"
                />
                <p className="text-xs text-muted-foreground">
                  {t('mcpAddServer.argsDescription') || 'Space-separated command arguments'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="stdio-scope">{t('mcpAddServer.scope') || 'Scope'}</Label>
                <SelectComponent
                  value={stdioScope}
                  onValueChange={(value: string) => setStdioScope(value)}
                  options={[
                    { value: "local", label: t('mcpAddServer.scopeLocal') || "Local (this project only)" },
                    { value: "project", label: t('mcpAddServer.scopeProject') || "Project (shared via .mcp.json)" },
                    { value: "user", label: t('mcpAddServer.scopeUser') || "User (all projects)" },
                  ]}
                />
              </div>
        ...
        <TabsContent value="sse" className="space-y-6">
          <Card className="p-6 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="sse-name">{t('mcpAddServer.name')}</Label>
                <Input
                  id="sse-name"
                  placeholder={t('placeholders.serverName')}
                  value={sseName}
              ...
              <div className="space-y-2">
                <Label htmlFor="sse-url">{t('mcpAddServer.sseUrl')}</Label>
                <Input
                  id="sse-url"
                  placeholder={t('placeholders.sseEndpoint')}
                  value={sseUrl}
                  onChange={(e) => setSseUrl(e.target.value)}
                  className="font-mono"
                />
                <p className="text-xs text-muted-foreground">
                  {t('mcpAddServer.sseUrlDescription') || 'The SSE endpoint URL'}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="sse-scope">{t('mcpAddServer.scope') || 'Scope'}</Label>
                <SelectComponent
                  value={sseScope}
      ...
      <Card className="p-4 bg-muted/30">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span>{t('mcpAddServer.exampleCommands') || 'Example Commands'}</span>
          </div>
