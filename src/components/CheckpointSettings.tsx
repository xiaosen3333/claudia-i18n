...
export const CheckpointSettings: React.FC<CheckpointSettingsProps> = ({
  ...
  const strategyOptions: SelectOption[] = [
    { value: "manual", label: t('checkpointSettings.strategyManual') },
    { value: "per_prompt", label: t('checkpointSettings.strategyPerPrompt') },
    { value: "per_tool_use", label: t('checkpointSettings.strategyPerToolUse') },
    { value: "smart", label: t('checkpointSettings.strategySmart') },
  ];
  ...
  const loadSettings = async () => {
    try {
      ...
      setError(t('checkpointSettings.failedToLoadSettings'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setSuccessMessage(t('checkpointSettings.settingsSaved'));
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
      console.error("Failed to save checkpoint settings:", err);
      setError(t('checkpointSettings.failedToSaveSettings'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleCleanup = async () => {
    try {
      ...
      setSuccessMessage(t('checkpointSettings.removedCheckpoints').replace('{count}', removed.toString()));
      setTimeout(() => setSuccessMessage(null), 3000);
      ...
      setError(t('checkpointSettings.failedToCleanup'));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {checkpointStrategy === "manual" && t('checkpointSettings.strategyManualDescription')}
            {checkpointStrategy === "per_prompt" && t('checkpointSettings.strategyPerPromptDescription')}
            {checkpointStrategy === "per_tool_use" && t('checkpointSettings.strategyPerToolUseDescription')}
            {checkpointStrategy === "smart" && t('checkpointSettings.strategySmartDescription')}
          </p>
        </div>
      ...
      <div className="border-t pt-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">
              {t('checkpointSettings.totalCheckpoints')} {totalCheckpoints}
            </p>
          </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {t('checkpointSettings.cleanUpDescription')} {keepCount}
          </p>
        </div>
      </div>
    </motion.div>
