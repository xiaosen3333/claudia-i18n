...
const WebviewPreviewComponent: React.FC<WebviewPreviewProps> = ({
  ...
  return (
    <div 
      ...
      tabIndex={-1}
      role="region"
      aria-label={t('webviewPreview.webPreview')}
    >
      {/* Browser Top Bar */}
      <div className="border-b bg-muted/30 flex-shrink-0">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{t('webviewPreview.title')}</span>
            {isLoading && (
            ...
            {onToggleMaximize && (
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    {isMaximized ? t('common.close') + " (ESC)" : t('sessionOutput.fullscreen')}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
        ...
        {hasError ? (
          <div className="flex flex-col items-center justify-center h-full p-8">
            ...
            <p className="text-sm text-muted-foreground text-center mb-4">
              {errorMessage || t('errorBoundary.description')}
            </p>
          ...
          <div className="flex flex-col items-center justify-center h-full p-8 text-foreground">
            ...
            <h3 className="text-xl font-semibold mb-3">{t('webviewPreview.noUrlProvided')}</h3>
            <p className="text-sm text-muted-foreground text-center mb-6 max-w-md">
              {t('webviewPreview.noUrlProvided')}
            </p>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t('webviewPreview.tryEntering')}</span>
              <code className="px-2 py-1 bg-muted/50 text-foreground rounded font-mono text-xs">localhost:3000</code>
              <span>{t('webviewPreview.orAnyOtherUrl')}</span>
            </div>
          </div>
        )}
      </div>
