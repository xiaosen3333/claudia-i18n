...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError(t('agents.failedToDelete'));
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('agents.created'), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('agents.updated'), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: `${t('agents.exportSuccess')}: "${agent.name}"`, type: "success" });
    } catch (err) {
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t('agents.importFromFile'), type: "success" });
      await loadAgents();
    } catch (err) {
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
                              ...
                              <Card className="h-full hover:shadow-lg transition-shadow">
                                <CardContent className="p-6 flex flex-col items-center text-center">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    {t('agents.createdAt')}: {new Date(agent.created_at).toLocaleDateString()}
                                  </p>
                                </CardContent>
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t('agents.importFromGitHub'), type: "success" });
        }}
      />
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('agents.deleteConfirm')} "{agentToDelete?.name}"? 
